...
def main() -> None:
    ...
    def apply_line_pid(patrol_speed: int, current_heading: float, target_heading: float) -> bool:
        ...
        correction = max(min(correction, 22.0), -22.0)



        steering_speed = clamp_speed(max(18.0, patrol_speed - min(abs(correction) * 1.4, 18.0)))

        if correction > 0:

            motor_controller.turn_right(steering_speed)

            set_status_summary(f"PID derecha {error:.2f}°")

        else:

            motor_controller.turn_left(steering_speed)
    ...
    def handle_line_follow_logic(patrol_speed: int) -> None:
        ...
        if center == 1:
            ...
            if target_heading is not None and current_heading is not None:
                ...
                if apply_line_pid(patrol_speed, float(current_heading), float(target_heading)):
                    ...
                    set_status_summary(f"Corrigiendo linea {error:.2f}?")
        ...
        while not shutdown_event.is_set():
            ...
            if snapshot.state == RobotState.IDLE:
                ...
                if snapshot.manual_direction == "backward" and is_reverse_motion_blocked():
                    pass
                elif snapshot.manual_direction in {"forward", "forward_left", "forward_right"} and is_front_motion_blocked():
                    pass
                elif snapshot.manual_direction != "stop":
                    apply_manual_move(snapshot.manual_direction, snapshot.manual_speed)
