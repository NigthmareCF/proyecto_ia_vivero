...
### Modulo ITG/MPU HW-123 (IMU I2C)
- **VCC:** Pin fisico 1 o 17 (+3.3V recomendado)
- **GND:** Pin fisico 6, 9, 14, 20, 25, 30, 34 o 39
- **SDA:** GPIO2, pin fisico 3 (bus I2C compartido con LCD)
- **SCL:** GPIO3, pin fisico 5 (bus I2C compartido con LCD)
- **AD0:** GND para direccion I2C `0x68`; opcional a +3.3V para `0x69`
- **INT:** Opcional a GPIO4, pin fisico 7, solo si se necesita interrupcion por software
- **Conflicto:** No hay conflicto de GPIO si se usa I2C compartido. Si el modulo se alimenta a 5V y trae pull-ups a 5V en SDA/SCL, si hay riesgo para la Raspberry Pi; por eso se recomienda alimentarlo a 3.3V.
...
## 4.5b Modulo ITG/MPU HW-123 (IMU I2C)

### Tabla de conexion sugerida para medicion continua de angulo X

| Pin HW-123 | Conexion recomendada en Raspberry Pi 5 | Pin fisico Pi 5 | Obligatorio | Uso |
|---|---|---:|---|---|
| **VCC** | +3.3V | 1 o 17 | Si | Alimentacion segura para mantener el bus I2C a 3.3V |
| **GND** | GND comun | 6, 9, 14, 20, 25, 30, 34 o 39 | Si | Referencia electrica comun |
| **SCL** | GPIO3 / SCL1 | 5 | Si | Reloj I2C compartido con la LCD |
| **SDA** | GPIO2 / SDA1 | 3 | Si | Datos I2C compartidos con la LCD |
| **XDA** | Sin conectar | - | No | Bus auxiliar del MPU; no se necesita para medir angulo |
| **XCL** | Sin conectar | - | No | Bus auxiliar del MPU; no se necesita para medir angulo |
| **AD0** | GND | Cualquier GND | Recomendado | Deja el MPU en direccion I2C `0x68`; a +3.3V cambia a `0x69` |
| **INT** | GPIO4 | 7 | Opcional | Interrupcion para muestreo; puede dejarse libre si el software consulta por polling |

**Decision sugerida, no definitiva:** para lectura continua del angulo X a `20-50 Hz`, usar el bus I2C principal compartido con la LCD. No hace falta crear otro bus I2C. Antes de cerrar el cableado, medir SDA/SCL: si la LCD alimentada a 5V eleva el bus por encima de 3.3V, agregar un conversor de nivel logico I2C bidireccional o retirar los pull-ups a 5V del modulo LCD.

```text
RASPBERRY PI 5                  ITG/MPU HW-123
----------------                  --------------

Pin 1 o 17  +3.3V  ------------> VCC
Pin 6/9/14/20/25/30/34/39 GND -> GND
GPIO2  pin fisico 3  SDA ------> SDA
GPIO3  pin fisico 5  SCL ------> SCL
GND                         ---> AD0  (direccion I2C 0x68)
GPIO4 pin fisico 7          ---> INT  (opcional, solo si se usa interrupcion)

BUS I2C COMPARTIDO:
  - LCD I2C: usa GPIO2/GPIO3, direccion tipica 0x27 o 0x3F
  - ITG/MPU HW-123: usa GPIO2/GPIO3, direccion tipica 0x68
  - No hay conflicto porque I2C permite varios dispositivos en el mismo bus.

ADVERTENCIA ELECTRICA:
  - Alimentar el HW-123 a 3.3V para mantener SDA/SCL a nivel seguro.
  - No alimentar a 5V si el modulo tiene pull-ups a VCC en SDA/SCL.
  - Si se alimenta a 5V y hay pull-ups a 5V, usar conversor de nivel logico.
```
...
| ITG/MPU HW-123 (VCC) | 3.3V | Pin físico 1 o 17 | Evita pull-ups I2C a 5V |
| ITG/MPU HW-123 (SDA/SCL) | 3.3V | GPIO2, GPIO3 (Pi 5) | Bus I2C compartido con LCD |
| ITG/MPU HW-123 (INT) | 3.3V | GPIO4, pin físico 7 | Opcional; sin uso si no se programa interrupción |
| **Indicadores locales** | | | |
...
## Paso 5c: Modulo ITG/MPU HW-123

```
□ VCC a +3.3V (pin fisico 1 o 17)
□ GND a tierra comun
□ SDA a GPIO2 / pin fisico 3 (mismo bus I2C que LCD)
□ SCL a GPIO3 / pin fisico 5 (mismo bus I2C que LCD)
□ AD0 a GND para direccion 0x68
□ INT opcional a GPIO4 / pin fisico 7, solo si se programa interrupcion
□ Verificar con i2cdetect -y 1: LCD en 0x27/0x3F y HW-123 en 0x68
```
...
INTERFAZ:
  GPIO2  → LCD SDA
  GPIO3  → LCD SCL
  GPIO2  → ITG/MPU HW-123 SDA (I2C compartido)
  GPIO3  → ITG/MPU HW-123 SCL (I2C compartido)
  GPIO4  → ITG/MPU HW-123 INT opcional
  GPIO21 → Buzzer
